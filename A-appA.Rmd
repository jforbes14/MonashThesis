...
For each election, we compute all possible five variable linear models, from which Akaike weights are computed. *Five* variable models are a compromise between computational burden and capturing sufficient information. 

It is too burdensome to run linear models for all possible combinations of our predictors. Instead an iterative procedure is employed to sample from the possible linear models, of all sizes. This is done to see where incremental gain in R-squared from adding a variable plateaus. 
...
3. Fit all possible three variable models that nest these 20 two-variable models, by adding one variable. This means for every retained model, there will be $28$ models fit.
...
Figure \@ref(fig:tradeoff) shows the R-squared for each of the models retained, with the number of predictors on the x-axis, and R-squared on the y-axis. The line represents the maximum R-squared each number of predictors. Around 70% of variation in two-party preference can be explained by a five variable model in each election, and the full variable set explains around 80% of variation (i.e. R-squared of 0.8). From variable 4 onwards, there is little incremental gain in R-squared from adding another variable.
...
The sum of Akaike weights $s_j$ for each variable $j$ in each of the elections is shown in the Table E.1 below.
...
print(tab_aw, comment = FALSE, include.rownames = FALSE, size = "\\scriptsize")
```
...
data_mod %>% ggplot(aes(x = CurrentlyStudying, y = Perc_LNP)) + geom_point(aes(label = Election_Division)) + geom_smooth(method = "lm") + facet_wrap(~year) + labs(x = "Proportion of population currently studying", y = "Two-party preferred (%)")
```
...
There are is little evidence of residual non-normality in the histograms (Figure \@ref(fig:histograms)) and quantile-quantile plots (Figure \@ref(fig:QQplots)), nor are there signs of residual heteroskedasticity (Figure \@ref(fig:reshet)). We produced plots of residuals against predictors in the model, which did not reveal any significant non-linear relationships. Doing the same for residuals against predictors excluded from the model, we do not find evidence of significant relationships between omitted socio-demographics and the residuals. All of this validates our assumptions about the distribution of the errors, and our model specification.
...
Testing for significant non-linearities in the relationships between the predictors in the model and the residuals, by fitting a loess smooth with a 95% confidence band. If zero is contained in the confidence band for all values of the predictor, then there are no significant non-linearities. No obvious non-linearities are found in any election.

```{r, fig.cap = "Residuals vs. predictors in the model. No obvious signs of non-linearity."}
aug_all %>%
  filter(year == "2016") %>% 
  gather(key = "variable", value = "value", -c(starts_with("."), Perc_LNP, year)) %>% 
  ggplot(aes(x = value, y = .resid)) +
  geom_point() + geom_smooth() + facet_wrap(~variable, scales = "free_x", nrow = 4) +
  labs(x = "Socio-demographic variable", y = "Residuals")
```

## Residuals vs predictors not in model {#a:patternsout}

Scatterplots of the residuals against socio-demographic predictors that are not included in the superset are shown below. A loess smooth is fit and overlaid in each scatterplot. No significant relationships are found between the residuals and predictors not in the superset.

```{r, fig.cap = "Residuals vs. predictors in the model. No significant relationships are found between the residuals and a predictor not in the superset."}
...
aug_all %>% bind_cols(data_mod %>% filter(!is.na(Perc_LNP))) %>% 
  ...
  ggplot(aes(x = value, y = .resid)) +
  geom_point() + geom_smooth(level = 0.95) + facet_wrap(~variable, scales = "free", nrow = 5) +
  labs(x = "Socio-demographic variable", y = "Residuals")
```

## Influence {#a:influence}

Across the six elections, only two electorates have moderately large influence - Kalgoorlie (2001, 2004) and Sydney (2007, 2016) - as they have a cook's distance greater than 0.2. The Kalgoorlie electorate supports the Liberal party  in these years ($TPP$ of around 55%), but has an extremely low incidence of administrative jobs and a high incidence of atheism which are strong factors of Labor support. Sydney is a staunch Labor electorate, with $TPP$ ranging from 30.50% to 35.35% across the six years, whilst having very large student populations, a low incidence of single parent households and high rental/loan prices. All of the observations with moderately large influence have hat values over 0.3, indicating that these also have unusual socio-demographic compositions.

The most influential electorate is Kalgoorlie in 2001, with cook distance of 0.31. As this is not particularly high (using a threshold of 0.5), it does not appear that there are any outliers that have high influence on the model, and hence no observations need to be removed. 
