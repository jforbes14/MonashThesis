...
As in section \@ref(ch:Data), the violin plots in figures \@ref(fig:unex1) and \@ref(fig:unex2) summarise the non-standardized socio-demographics that are *not* included the final models, and figure \@ref(fig:scex) summarises the standardized. Each violin depicts the distribution as a density plot reflected in the vertical axis.

```{r unex1, fig.cap = "Part 1: Distribution of unscaled variables excluded from the models."}
# Unscaled violin plots go here
 profiles %>% 
  select(-c(NoReligion, CurrentlyStudying, MedianAge, OneParentHouse, BornUK, BornSEEurope, ExtractiveJobs, AdminJobs, HighSchool, Bachelor, Postgraduate, MedianRent, MedianLoanPay, Election_Division, Swing, Perc_LNP, PartyAb)) %>%
  select(starts_with("Age"), Anglican, Area, AusCitizen, AverageHouseholdSize, BornAsia, BornElsewhere, BornMidEast, Buddhism, Catholic, Christianity, Couple_NoChild_House, Couple_WChild_House, DeFacto, DiffAddress, DipCert, Distributive, year) %>% 
...
```{r unex2, fig.cap = "Part 2: Distribution of unscaled variables excluded from the models."}
# Unscaled violin plots go here
 profiles %>% 
  select(-c(NoReligion, CurrentlyStudying, MedianAge, OneParentHouse, BornUK, BornSEEurope, ExtractiveJobs, AdminJobs, HighSchool, Bachelor, Postgraduate, MedianRent, MedianLoanPay, Election_Division, Swing, Perc_LNP, PartyAb)) %>%
  select(-c(starts_with("Age"), Anglican, Area, AusCitizen, AverageHouseholdSize, BornAsia, BornElsewhere, BornMidEast, Buddhism, Catholic, Christianity, Couple_NoChild_House, Couple_WChild_House, DeFacto, DiffAddress, DipCert, Distributive)) %>% 
  gather(key = "metric", value = "value", -year) %>%
  ggplot(aes(x=year, y=value)) +
  geom_violin(aes(fill = year)) + 
  facet_wrap(~metric, scales = "free_y", ncol = 6) +
...
```{r scex, fig.cap = "Distribution of unscaled variables (including factors) excluded from the models."}
