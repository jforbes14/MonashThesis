...
```{r resvpred, out.width='80%', fig.cap = "Residuals vs. predictors in the 2016 election model. No obvious signs of non-linearity."}
aug_all %>%
  filter(year == "2016") %>% 
...
There are twenty socio-demographic predictors that are not in the superset, and some of these may seem releveant and useful to include in a model. However, most of the variation in many of the left out variables is explained by predictors included in the model. To determine the percentage of variation explained, the R-squared is recorded from a regression of each left out predictor on the superset of explanatory variables in the model (including interactions). Figure \@ref(fig:ovb) shows these R-squared values, for each left out predictor, in each election.
