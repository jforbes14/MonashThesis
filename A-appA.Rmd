...
It is too burdensome to run linear models for all possible combinations of the $p=30$ predictors. Instead an iterative procedure is employed to sample from the possible linear models, of all sizes. This is done to see where the incremental gain in R-squared from adding a variable plateaus. 
...
```
```{r TPPviolin, out.width='75%', fig.cap="Violin plots illustrating the distribution of the two-party preferred vote across election years."}
data_mod %>% 
  ggplot(aes(x = year, y = Perc_LNP)) + 
  geom_violin(aes(fill = year)) +
  labs(x = "Election year", y = "Two-party preferred vote (%)") +
  theme(axis.text.x = element_text(size = 8, angle = 75, vjust = 0.5), 
        axis.text.y = element_text(size=8)) + coord_fixed(0.05) + guides(fill = FALSE)
```
