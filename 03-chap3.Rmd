...
```{r hume13, fig.cap = "Electoral boundaries in NSW for 2013, with the electoral boundary for Hume, shown in purple."}
# 2013
...
Mapping_aec13 %>% 
  ...
  xtable() %>% 
  print(comment=FALSE, digits=NULL, size = "\\small", include.rownames = FALSE)
```
...
```{r vis-relig, out.height='25%', fig.cap="Distribution of no religion across Australian electorates, by election year. Over time, the proportion of electoral populations not identifying with a religion has increased."}
profiles %>% 
...
In this chapter we have detailed the steps of data collection, preliminary transformations and imputation that result in a cross-sectional data set of electoral socio-demographics and two-party preferred vote for each of the six elections. Violin plots summarizing the full set of socio-demographic variables can be found in Appendix \@ref(a:summary). Socio-demographic (explanatory) variables are standardized so that the data scales are consistent across years, which allows for comparison of effects across elections. The next steps are to determine an appropriate statistical model, and devise a strategy to deal with the problem of having a small number of observations each election ($n=150$) relative to the number of available predictors ($p=63$). This is explored in Chapter \@ref(ch:Modelling).

[^4]: Note that in 2001, median age is rounded to the nearest integer.