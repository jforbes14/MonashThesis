...
For many of the socio-demographic variables, both the sample mean and variance each change over the years. Consider median rental price in each electorate, for example. In 2001, the median rental price amongst electorates had mean $161.83$ and standard deviation $50.89$, whereas in 2016 the mean was $231.49$ and the corresponding standard deviation being $61.88$. These figures were adjusted for inflation using a 2001 index. Since each election is to be modelled separately and then compared, each variable is centered to have mean $0$, and standardized to have variance $1$, within each election year, so that the data scales are consistent across years.

The values for an electorate can therefore be interpreted as a score relative to the other electorates in that election. For example, a value of $AusCitizen = 1$ corresponds with an electorate having a proportion of Australian citizens that is one standard deviation above the average in that election.
