...
We propose an innovative spatial imputation algorithm using GIS maps to impute the demographic profiles for each election. The algorithm accounts for both temporal and spatial differences of the electorate, relative to the Census divisions in Censuses either side of the election. The use of GIS maps to overlay data from multiple sources is a dominant approach in spatial studies, which provides the foundation for our approach..

It is worth noting that, on average, approximately 20% of division boundaries change by 25% or more between Census and election times, so directly allocating Census information from a Census electorate to an election electorate may ultimately lead to invalid results. This provides further motivation.
...
ggplot(data=map_hume_impute %>% 
  ...
  theme(legend.position="bottom", plot.title = element_text(face = "bold", size = 18, hjust = 0.5),
        axis.title = element_text(size = 12), axis.text = element_text(size = 16)) + 
  #facet_wrap(~part) + 
...
Now that we have imputed socio-demographic profiles for the 2004, 2007, 2010 and 2013 elections, we have a complete collection of data for every electorate in every election. The full set of definitions for socio-demographic variable sare detailed in appendix A.
...
Summary statistics for the combined data set for all elections is shown in \@ref(fig:summaryall). An individual summary for each election can be found in appendix E.
...
```{r summaryall, results = 'asis'}
stargazer::stargazer(data_mod %>% filter(!is.na(Perc_LNP)), title = "Summary statistics for each socio-demographic variable, and the response, across all elections.", header = FALSE)
```
