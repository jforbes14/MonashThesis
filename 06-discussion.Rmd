...
Our findings complement the existing literature by modelling temporal trends, which has not been previously done using a regression framework. Furthermore, as far as we can see, no study has modelled an election from 2010 using Census information, whereas we have done so for the most recent six. This research is therefore a significant contribution, helping uncover the dynamics of Australian voter behaviour in the 21st century, and by providing a resource for future analysis.
...
?????
Although not included here, the same process In an attempt to improve imputation accuracy, we 
