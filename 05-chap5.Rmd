...
```{r SE1, out.width='100%', fig.cap="Partial residual plots for the effect of population born in South-Eastern Europe, across election years. A linear regression line is overlayed to illustrate the decreasing relationship."}
...
```{r SE2, out.width='75%', fig.cap="Estimated coefficients for the population born in South-Eastern Europe, across election years and with 95\\% confidence bands. A significant negative effect is found in each election."}
# Coefficients
...
```{r norelig1, fig.cap="Partial residual plots for the effect of the population proportion not identifying with a religion, across election years. The overlayed regression lines show that a significant negative effect emerges in 2010."}
...
```{r norelig2, out.width='75%', fig.cap="Estimated coefficients for population proportion not identifying with a religion, across election years and with 95\\% confidence bands. From 2010 onwards, the effect is significant."}
# Coefficients
...
```{r studying, fig.cap="Partial residual plots for the effect of student populations, across election years. A positive effect is estimated from 2007, but is only significant in 2010."}
...
```{r studying2, out.width='75%', fig.cap="Estimated coefficients for population currently studying, across election years and with 95\\% confidence bands. The estimated effect shifts from negative to positive, but is only significant in 2010."}
# Coefficients
...
```{r educage, fig.cap="Raster plot of combinations of education and median age, in each election year. Colour indicates the predicted two-party preferred vote. Electorates with high education levels and low median age consistently support Labor (bottom-right). Older electorates with low education levels shift to support Liberal from 2007 (top-left)."}
...
```{r educOP, fig.cap="Raster plot of combinations of education levels and the proportion of single parent households, in each election year. Colour indicates the predicted two-party preferred vote. For all education levels, a higher proportion of single parent households shifts predicted support to Labor."}
...
```{r jobs1, fig.cap="Raster plot of combinations of extractive jobs and administrative jobs, in each election year. Colour indicates the predicted two-party preferred vote. Electorates with many workers in both job types are predicted to support Liberal, whilst those with few workers in both job types support Labor."}
jobs_df <- data.frame(
...
In each election, an electorate with the combination of an above-average proportion of administrative jobs and low education levels is predicted to strongly support the Liberal party. Otherwise, an electorate is predicted to support Labor. This is consistent across elections. Raster plots for this interaction can be found in Appendix \@ref(a:extrainsights).

## Summary

The estimated main effects and two-way interactions for each election have been illustrated in this section. We find that electoral socio-demographics have consistent relationships with the two-party preferred vote over the six elections. Notable exceptions are the effect of `NoReligion`, and the combined effect of being an older electorate with low education levels.

\newpage

## Typical electorates

Based on the model for the most recent election (2016), hypothetical electorates are constructed in Table 5.1 to represent the socio-demographics that are predicted to have the strongest support for each of the Labor and Liberal parties.

\begin{table}[!htbp] \centering 
\caption{Characteristics of a hypothetical electorate that is predicted to strongly support each party in the 2016 election.}
\normalsize
\begin{tabular}{@{\extracolsep{5pt}}lll} 
\\[-1.8ex]\hline 
\hline \\[-1.8ex] 
 & \multicolumn{2}{c}{\textit{Typical electoral characteristics in 2016:}} \\ 
\cline{2-3}
\\[-1.8ex] & \bf{Labor} & \bf{Liberal} \\ 
\hline \\[-1.8ex] 
\bf{Age} & Young & Old \\ 
\bf{No Religion} & Many & Few \\
\bf{Immigrants} & Many & Few \\
\bf{Education} & High & Low \\
\bf{Student population} & Low & High \\
\bf{Rent and loan payments} & Low & High \\
\bf{Extractive jobs} & Few & Many \\
\bf{Administrative jobs} & Few & Many \\
