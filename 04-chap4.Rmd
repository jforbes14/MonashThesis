...
When doing any kind of variable selection, omitted variable bias is common concern. By construction, we have not omitted any of the key variables in each election, meaning that any left out predictors should have little or no effect on the two-party preferred vote, over and above what is included in the model. 

On the surface, it may seem that variables like incomes and labor force participation should be relevant and included in the model. However, we find over 85% of variation in incomes and over 75% of variation in labor force participation is explained by the superset of variables. This is obtained by regressing each omitted variable on the superset of variables (see appendix H). 

Therefore, we are confident that our model specification is not strongly affected by omitted variable bias.
