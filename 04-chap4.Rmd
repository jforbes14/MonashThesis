...
The two-party preferred vote is a measure of preference between only the two major parties (Labor and Liberal), which is orientied in favour of the Liberal party and sits in the interval $(0,100)$. For example, $TPP = 0.7$ represents a 70% preference for Liberal, 30% for Labor. Since the values that $TPP$ assumes are not close to 0 or 100 (minimum $24.05 \%$ and maximum $74.90 \%$), we do not need to worry about modelling the constraint of $TPP \in (0,100)$, instead we can model the response using linear regression.

Therefore, for a given election year, we assume $TPP \sim N(X \beta, \sigma^2)$, where $TPP$ is an $n \times 1$ vector of two-party preferred vote, $X$ is an ($n \times p$) matrix of socio-demographic predictors, $\beta$ is a ($p \times 1$) vector of coefficients to be estimated, and errors are independent and identically distributed with mean $0$ and variance $\sigma^2$. We estimate $\beta$ and $\sigma^2$ using ordinary least-squares regression, and conduct inference on the output. This estimation is done separately for each election.
...
Instead of throwing everything into a model and using some kind of procedure to reduce the dimension of the data (e.g. stepwise or LASSO regression), we decide to adopt a two-stage approach in obtaining a reduced predictor set, from which we will run our final election models. 

The first step is to use principal component analysis (PCA) to identify variables that are correlated, and group them together into *factors*. Variable groupings must have a reasonable interpretation in order to be combined into a factor. This reduces our predictor set from $p = 63$ to $p = 30$, from which we still need to do some kind of variable selection, so that we end up with a set of around $10$ variables from which we can model each election.
...
- Education levels: $ \text{Education = Bachelor + HighSchool + Postgraduate + Professional + Finance - Laborer - Tradesperson - DipCert} $

- Family and house size: $ \text{FamHouseSize = FamilyRatio + AverageHouseholdSize + Couple-WChild-House - Couple-NoChild-House - SP-House + Age00-04 + Age05-14 } $

- Property ownership and marriage rates: $\text{PropertyMarr = Married + Owned + Mortgage - Renting - DiffAddress - PublicHousing - DeFacto}$

- Income: $\text{Incomes = MedianFamilyIncome + MedianHouseholdIncome + MedianPersonalIncome}$

- Rental and loan payments: $\text{RentLoan = MedianLoanPay + MedianRent}$

After computing these sums, each factor is again standardized to have mean 0 and variance 1 within each election.

Consider the `Incomes` factor as an illustration. Independent of principal components, we may suspect that median personal income, median household income and median family income are providing similar information about how "well-off" an electorate is. When we look at the loadings in the first principal component (see figure \@ref(fig:PC1)), we find that these three variables all have large loadings. This provides evidence that these variables could be combined in a factor, which we have called `Incomes`.
...
To estimate a *good* model, we aim to produce a superset that captures the five *most important* variables from each election. We adopt the approach by @BurnhamAnderson2002, using Akaike weights to measure relative variable importance. This is fundamentally based on Akaike's information criterion (AIC) [@Akaike73] and is common in ecological studies. 

AIC is a measure of the expected information lost in estimating the true underlying data generating process, which is why we choose a model with minimum AIC when comparing models in a given set $M$. Akaike weights are a way of scoring each model relative to the rest of the models in the set.
...
For each variable $j \in 1, ..., J$, we compute the sum of Akaike weights ($s_j$) over models that include that variable, and use these as a measure of variable importance [@BurnhamAnderson2002]. Let $I$ denote the indicator function.

$$s_j = \sum_{m=1}^{M} w_m \cdot \text{I}(\text{variable } j \text{ used in model } m)$$

The ordering of variables by their Akaike weights is a rank of variable importance. The variable with largest $s_jJ$ is deemed to be the most important in the set.
...
We have chosen to five variable *models* as a compromise between computational burden and capturing sufficient information. Five variables can capture around 70% of the variation in two-party preferred vote, and incremental gains of increasing the number of predictors begin to plateau around five variables (see appendix G for a full description). Furthermore, the number of models to fit becomes relatively infeasible quickly after five variables. 

Akaike weights $w_m$ are calculated and variables ranked by their relative variable importance $s_j$ for each election. Taking the top five variables from each election, the following superset is obtained (with the number of elections for which a variable is in the top five in parenthesis).
...
Two-way variable interactions are then added to the model using an iterative procedure. First we fit a model using the superset of predictors as main effects only. Then we compute the number of elections in which each possible two-way interaction significantly adds to the model fit, using a likelihood-ratio test (1% significance level). We then select the interaction that is significant in the most elections and re-fit models by adding this interaction to the previous model specification. 
...
When doing any kind of variable selection, omitted variable bias is a concern. By constructing the superset, we have not omitted any of the key variables from each election. Therefore any left out predictors should have little or no effect on the two-party preferred vote, over and above what is included in the model. 

On the surface, variables like incomes and labor force participation might seen relevant and useful inclusions in the model. However, over 85% of variation in incomes and over 75% of variation in labor force participation is explained by the superset of variables. This is obtained by regressing each omitted variable on the superset of variables (see appendix H). Other variables, Judiasm for example, are not well explained by the superset, but are also not influential in determining two-party preference.
