...
- Education levels: Educ = Bachelor + HighSchool + Postgraduate + Professional + Finance - Laborer - Tradesperson - DipCert
...
To estimate a *good* model, we aim to produce a superset that captures the five *most important* variables from each election. Our approach is based off a technique common in ecological studies, using Akaike weights to measure relative variable importance (@BurnhamAnderson2002).

Akaike's information criterion (AIC) (@Akaike73) is a measure of the expected information lost in estimating the true underlying data generating process. When comparing models in a given set $M$, we aim to minimize AIC. Akaike weights is a method of scoring each model, relative to the rest of the models in the set.
